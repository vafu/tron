...
use tron_api::{
    CaptureFormat, CapturedFrame, EncodedFormat, EncodedFrame, Frame, FrameMeta, FrameSource,
    FrameTimestamp, PixelFormat, SensorKind, TimestampSource,
};
use v4l::FourCC;
...
pub struct V4lFrameSource {
    config: V4lSourceConfig,
    stream: MmapStream<'static>,
    next_id: u64,
}

impl V4lFrameSource {
    pub fn open(mut config: V4lSourceConfig) -> Result<Self> {
        ...
        let requested_fourcc = fourcc(config.format);
...
fn frame_meta(id: u64, sensor: SensorKind, width: u32, height: u32, meta: &Metadata) -> FrameMeta {
    FrameMeta {
        id,
        sensor,
        width,
