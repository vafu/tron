...
impl MockLandmarker {
    pub fn new() -> Self {
        Self { pose: MockPose::Open, center: (0.5, 0.5), scale: 0.30 }
    }
}
...
fn canonical_pose(pose: MockPose, cx: f32, cy: f32, s: f32) -> [Vec3; 21] {
    ...
    let f = |x: f32, y: f32| Vec3 { x: cx + x * s, y: cy + y * s, z: 0.0 };
    // Open palm template.
    let mut p = [
        f( 0.00,  0.50),    // 0  wrist
    ...
    let curl = |p: &mut [Vec3; 21], mcp: usize, pip: usize, dip: usize, tip: usize| {
        let m = p[mcp];
        let toward = |a: Vec3, t: f32| Vec3 { x: a.x * (1.0 - t) + m.x * t, y: a.y * (1.0 - t) + m.y * t, z: 0.0 };
    ...
    match pose {
        ...
        MockPose::Pinch => {
            ...
            let mid = Vec3 { x: (p[4].x + p[8].x) * 0.5, y: (p[4].y + p[8].y) * 0.5, z: 0.0 };
            p[4] = mid;
            p[8] = mid;
        }
        MockPose::Point => {
        ...
        MockPose::ThumbsUp => {
            ...
            p[4] = Vec3 { x: cx - 0.30 * s, y: cy - 0.40 * s, z: 0.0 };
        }
    }
    p
}
