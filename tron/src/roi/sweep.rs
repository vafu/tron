...
impl RoiSweep {
    ...
    pub fn update(
        ...
        let mut rect = controller
            ...
            .clamp_to(frame_size);
        let max_x = frame_size.width.saturating_sub(rect.width);
        if self.direction >= 0 {
    ...
    pub fn maybe_log(&mut self, frame: Frame<'_>, roi: RoiRect, now: Instant) {
...
impl BrightnessStats {
    fn measure(frame: Frame<'_>, roi: RoiRect) -> Option<Self> {
        match frame.format {
...
fn gray8_stats(frame: Frame<'_>, roi: RoiRect) -> Option<BrightnessStats> {
    let size = frame.meta.size;
    let roi = roi.non_empty_or_default().clamp_to(size);
    ...
    for y in roi.y..roi.y + roi.height {
        ...
        let row_end = row_start + roi.width as usize;
    ...
    Some(BrightnessStats {
        roi_mean: roi_sum as f32 / (roi.width * roi.height) as f32,
